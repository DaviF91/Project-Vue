<template>
    <TransitionRoot :show="Store.actionButtonSelected == 'Editar Forma de Pagamento'" as="template">
                ...
                <div class="flex min-h-full items-center justify-center p-4 text-center"> 
                    <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95" style="max-width: 79rem;" enter-to="opacity-100 scale-100" leave="duration-200 ease-in" leave-from="opacity-100 scale-100" leave-to="opacity-0 scale-95">
                        <DialogPanel class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                            <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900 text-center"> Editar Forma de Pagamento </DialogTitle>
                            <button style="position: absolute;"></button>
                            <FormsEditarFormaPagamento @dados-alterados="handleDadosAlterados" />
...
<script>
  import FormsEditarFormaPagamento from './FormsEditarFormaPagamento.vue';

  export default {
    components: {
        FormsEditarFormaPagamento,
    },
    data() {
        return {
            dadosAppEditarFormaPagamento: [],
        };
    },
    methods: {
        handleDadosAlterados(dados) {
            this.dadosAppEditarFormaPagamento = dados;
            this.dadosAppEditarFormaPagamento.IdFormaPagamentoBase = parseInt(this.dadosAppEditarFormaPagamento.IdFormaPagamentoBase);
            console.log('Dados Alterados no componente:', this.dadosAppEditarFormaPagamento);
        },
        salvarDados(){
            const Store = useStore();
            const id = Store.idSelecionadoFormaPagamento
            const apiUrl = `/api/teste/FormaPagamentoContasPagar/Put/${id}`;
            ...
            fetch(apiUrl,{
                ...
                body: JSON.stringify(this.dadosAppEditarFormaPagamento),
            })
