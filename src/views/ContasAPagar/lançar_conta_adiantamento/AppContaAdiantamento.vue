<template>
    <TransitionRoot :show="Store.actionButtonSelected == 'Lançar Conta com Adiantamento'" as="template">
                ...
                <div class="flex min-h-full items-center justify-center p-4 text-center"> 
                    <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95" style="max-width: 79rem;" enter-to="opacity-100 scale-100" leave="duration-200 ease-in" leave-from="opacity-100 scale-100" leave-to="opacity-0 scale-95">
                        <DialogPanel class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                            <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900 text-center"> Lançamento de Conta com Adiantamento</DialogTitle>
                            <FormsLancarConta />
                            <InformarPagamento />  
...
import InformarPagamento from '../informar_pagamento/FormsInfoPagamento.vue'
import FormsLancarConta from '../lançar_conta/FormsLancarConta.vue'
